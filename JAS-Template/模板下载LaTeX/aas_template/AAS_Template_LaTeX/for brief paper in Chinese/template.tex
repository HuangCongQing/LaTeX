...
\thanks{Manuscript received Month Date, Year;
...
\cnabstract{中文摘要字数应为文章总字数的3\,\%左右,一般不超过100字.
摘要应涵盖全文.摘要内容包括研究目的、方法、结果等,注意不是标题的罗列,
能独立成文,不能出现公式号和文献号.英文摘要的书写,请按英语习惯,
无文法及拼写错误,用词准确.}
...
\entitle{Preparation of Papers for\\\vskip 0.2\baselineskip Acta
Automatica Sinica}
...
本文是《自动化学报》中文稿件\LaTeX 模版的一个样例和使用说明.
模版的中文支持部分采用CCT. 这一章为引言,无需写标题.

\section{准备稿件}

在正文开始之前,需要定义\verb|\cntitle|来定义中文标题,用\verb|\cnauthor|来定义中文作者,用\verb|\cnaddress|来定义中文地址,
用\verb|\cnabstract|来定义中文摘要,用\verb|\cnkeyword|来定义中文关键词;
...
\begin{verbatim}
LaTeX Font Warning: Some font shapes were not
available, defaults substituted.
\end{verbatim}
请依次检查: 1) 您的\LaTeX 系统是否已经安装了您使用的字体; 2)
字体的引用名称是否
正确,比如仿宋体的名称为fangsong,而不是fs,此时需要相应的修改aas.cls
文件第64\,$\sim$\,69行.

\subsection{{\bf pdf}~的生成方法}

编译生成顺序如下: 1) 运行CCT \& Latex命令两次生成dvi和ps文件; 2)
使用dvi2pdf编译dvi文件生成pdf文件.
...
定稿时论文和综述版面不超过七页;短文不超过四页; 长论文版面可稍放宽.
...
W}、{\bf X}、{\bf Y}、{\bf Z}、${\pmb a}$~和\linebreak
\onecolumn\begin{multicols}{2}\noindent%这一行用于第一页末尾



\noindent ${\pmb\alpha}$、${\pmb
...
etc., e.g., s.t.排正体.式(1)、(2)是两个简单单栏数学公式的例子.
\begin{equation}
\begin{pmatrix}
...
式(3)和(4)是通栏的公式的例子. 可以用环境\verb|\end{multicols}|
\verb|\begin{multicols}{2}|产生.
...
proportion and integral parameters of the
Consarc controller }}}
\end{center}
\end{verbatim}
...
\vbox{\centerline{\psfig{figure=1.eps,width=5.5cm}} \vskip1mm
...
参考文献要按在文中出现的先后依次编号$1,2,\cdots$.所引文献在正文中务请标注.
参考文献中不使用任何缩略语.作者人数超过六人以上可用``et al.'',否则
请注明全部作者姓名.注意文献条目中各项内容应该齐备,
...
\\
Table 1\quad An exampletable in one column} \vskip1mm
...
\begin{center}
{\centerline{\psfig{figure=2.eps,width=6.5cm}}
{\footnotesize (a)控制参数1\\(a) Controller parameter 1}} \\[2mm]
{\centerline{\psfig{figure=3.eps,width=6.5cm}} {\footnotesize
(b)控制参数2\\(b) Controller parameter 2}} \vskip2mm {\small
图2\quad 控制器参数对刀具运动轨迹的影响
\\Fig.\,2\quad The influences
of the controller parameters on the tracking errors}
\end{center}

作者照片请选用简单背景的证件照片,比例要求是$5:4$,大小做成3.0\,cm\,$\times$\,2.4\,cm.在无照片情况下,
采用biographynophoto环境来填写作者简历.
...
\begin{biographynophoto}
\noindent{\hei 张\hskip2.5mm哲}\quad 《自动化学报(英文版)》与《自动化学报》编辑部技术编辑.
本文通信作者. E-mail: zhe.zhang@ia.ac.cn

\noindent({\bf ZHANG Zhe}\quad Copy-editor at the Editorial Office of
{\sl IEEE/CAA Journal of Automatica Sinica (JAS)}, and {\sl
Acta Automatica Sinica}. Corresponding author of this paper.)
\end{biographynophoto}
\end{multicols}
\end{document}
